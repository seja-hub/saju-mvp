...
// 단체용 압축 컨텍스트
function memberLine(name, s, gender) {
  const d = s.오행분포;
  return `${name}(${gender || '성별미상'}): 일간 ${s.일간.hangul}(${s.일간.hanja}·${s.일간.오행}·${s.일간.음양}), 일지 ${s.일지.hangul} / 오행 목${d.목} 화${d.화} 토${d.토} 금${d.금} 수${d.수}`;
}
...
function buildPrompt(section, saju, extra) {
  ...
  if (section === '인연사주') {
    const inst = `이 사람은 오늘 썸·연애 컨셉의 솔로파티(사주 미팅 행사)에서 새로운 인연들을 만나. 이 사람의 '인연사주'를 아주 풍성하게 — 7개 문단, 문단당 4~6문장으로 풀어줘. 각 문단 앞에 ◆ 소제목을 붙이고: ⑴인연 자리에서 내 사주가 어떤 캐릭터인지 임팩트 있는 선언으로 시작 ⑵연애 스타일과 어필 포인트(십신 근거로 구체적으로) ⑶내가 끌리는 사람 vs 나에게 끌리는 사람 — 둘이 다를 수 있다는 걸 재밌게 ⑷잘 맞는 인연상 — 어떤 오행·기운의 사람이 내 귀인인지 구체적으로, 현장에서 바로 알아볼 관찰 포인트와 함께 ⑸◆ 에겐력·테토력 — 고정값의 %를 본문에 그대로 표기하고(절대 변경 금지) 왜 그렇게 읽히는지 사주 근거로 재밌게. MBTI는 절대 언급하지 말 것 ⑹◆ 연애 팩폭 — 이 사주가 연애에서 반복하는 패턴이나 스스로 연애를 망치는 습관 1~2가지를 사주 근거로 뼈 때리게 짚기. 밉지 않게, 친한 친구가 술자리에서 놓는 팩폭 톤으로("솔직히 말할게" 같은 도입 허용). 이어서 "그래서 오늘은 이렇게 해봐"로 사주 약점을 보완하는 당장 실천 행동 처방 1개를 구체적으로(예: 먼저 질문 3개 던지기, 리액션 아끼지 말기, 잔 채워주며 말 걸기) ⑺오늘의 인연운 — 아래 [오늘] 일진과 본인 일간의 관계를 근거로 + 모임 마무리 팁 하나. 톤: 솔로파티 참가자가 읽는 글이니 설레발과 위트를 아끼지 말 것 — "오늘 연락처 교환각", "첫 잔에 승부 봐야 하는 타입" 같은 현장감 있는 표현 환영. 단 비하·외모 언급·과한 술 권유는 금지.`;
    return `${SYSTEM}\n\n${extra.who ? extra.who + '\n' : ''}${sajuContext(saju)}\n\n${extra.todayLine}\n\n[에겐·테토 고정값 — 숫자는 절대 바꾸지 말 것]\n${extra.personaBlock}\n\n요청: ${inst}`;
  }
  if (section === '단체궁합') {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ALLOWED = ['profile','오늘의운세','이번달운세','총운','연애운','결혼운','금전운','직업운','건강운','궁합','에겐테토','인연사주','단체궁합','단체에겐테토','술값귀인'];
    ...
    if (isGroup) {
      ...
      extra.n = list.length;
      extra.memberBlock = list.map((m, i) => `${i + 1}. ${memberLine(m.name, m.saju, m.gender)}`).join('\n');
      if (section === '단체궁합') {
        const lines = [];
      ...
      if (section === '인연사주') {
        const pm = personaMetrics(saju);
        // 인연사주는 MBTI 미사용 — 에겐·테토 %만 고정값으로 주입
        extra.personaBlock = `${name || '이 사람'}: 에겐 ${pm.egen}% / 테토 ${pm.teto}%`;
        const kst = new Date(Date.now() + 9 * 3600 * 1000);
        const Y = kst.getUTCFullYear(), M = kst.getUTCMonth() + 1, D = kst.getUTCDate();
        const today = buildSaju({ year: Y, month: M, day: D });
        extra.todayLine = `[오늘] ${Y}-${String(M).padStart(2, '0')}-${String(D).padStart(2, '0')} · 일진 ${today.pillars.일주.hangul}(${today.pillars.일주.hanja})`;
        maxTok = 4200; // 7개 문단 × 4~6문장 분량
      }
    }
