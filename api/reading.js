...
function buildPrompt(section, saju, extra) {
  ...
  if (section === '인연사주') {
    const inst = `이 사람은 곧 사주로 인연을 찾는 미팅 행사에서 새로운 사람들을 만나. 이 사람의 '인연사주'를 다른 섹션보다 1.5배 깊게 — 6~7개 문단, 문단당 3~5문장으로 풀어줘. 각 문단 앞에 ◆ 소제목을 붙이고: ①내 사주가 '인연 자리'에서 어떤 캐릭터인지 임팩트 있는 선언으로 시작 ②연애 스타일과 어필 포인트(십신 근거로 구체적으로) ③내가 끌리는 사람 vs 나에게 끌리는 사람 — 둘이 다를 수 있다는 걸 재밌게 ④잘 맞는 인연상 — 어떤 오행·기운·일간을 가진 사람이 내 귀인인지 구체적으로, 모임 자리에서 바로 써먹을 수 있는 관찰 포인트와 함께 ⑤◆ 예상 MBTI·에겐력/테토력 — 고정값의 숫자와 MBTI를 본문에 그대로 표기하고(절대 변경 금지) 왜 그렇게 읽히는지 사주 근거로 짧고 재밌게 ⑥오늘의 인연운 — 아래 [오늘] 일진과 본인 일간의 관계를 근거로, 모임에서 바로 써먹을 실전 팁 하나로 마무리. 전체 톤은 캐주얼하고 위트 있게, 읽고 나면 옆 사람에게 보여주고 싶어지게.`;
    return `${SYSTEM}\n\n${extra.who ? extra.who + '\n' : ''}${sajuContext(saju)}\n\n${extra.todayLine}\n\n[에겐·테토/MBTI 고정값 — 숫자·MBTI는 절대 바꾸지 말 것]\n${extra.personaBlock}\n\n요청: ${inst}`;
  }
  if (section === '단체궁합') {
