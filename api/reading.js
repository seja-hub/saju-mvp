...
const SYSTEM = `너는 한국 명리학(사주팔자)에 빠삭한 사주 상담가야. 행사·술자리에서 사람들한테 재미로 봐주는 콘텐츠를 쓴다.
...
function buildPrompt(section, saju, extra) {
  ...
  if (section === '에겐테토') {
    return `${SYSTEM}\n\n${extra.who ? extra.who + '\n' : ''}${sajuContext(saju)}\n\n[판독 고정값 — 숫자와 MBTI는 절대 바꾸지 말 것]\n${extra.personaLine}\n\n요청: 요즘 유행하는 '에겐/테토' 판독을 사주로 해줬어. 위 고정값을 그대로 쓰되, '왜 그런 판정이 나왔는지'를 이 사주의 일간 기질·오행·십신과 엮어서 재밌게 풀어줘. 예: "부드러운 을목 기질에 인성까지 발달 — **따뜻한 마음의 에겐녀일 가능성 ${extra.egen}%**!" 같은 톤. ①첫 문단: 판정 결과를 임팩트 있게 선언 ②에겐/테토 판정의 사주적 근거 ③예상 MBTI 네 글자를 각각 사주 근거로 짧게 해설 ④이 성향이 연애·술자리에서 어떻게 드러나는지 ⑤마지막 한 줄: 이건 재미로 보는 '사주 유사과학'이라는 걸 위트 있게. 성별을 모르면 '에겐형/테토형'으로 불러.`;
  }
  if (section === '단체궁합') {
    return `${SYSTEM}\n\n[멤버 ${extra.n}명]\n${extra.memberBlock}\n\n[페어 케미 데이터 — 점수 높을수록 합이 좋음, '충'은 투닥거리는 자극 케미]\n${extra.pairBlock}\n\n요청: 이 ${extra.n}명이 지금 한 술자리에 모였어. 단체 케미를 재밌게 분석해줘. ①이 모임의 전체 분위기를 한두 문장으로(첫 문단, 임팩트 있게) ②**베스트 케미 TOP 3** — 어떤 페어인지와 이유(케미 점수·합·보완 근거) ③**환장의 조합** 1~2쌍 — 충이 있거나 점수가 낮은 페어를 밉지 않게 투닥 케미로 ④멤버별 오늘의 술자리 롤 — 전원 한 명당 한 줄씩, 사주 기질 근거로(분위기메이커·중재 담당·총무각·리액션 장인·갑자기 진지·끝까지 생존 등 재밌는 롤명 자유롭게). 한 문단 안에서 줄바꿈으로 나열 ⑤총평 — 오늘 이 자리가 어떻게 흘러갈지 짧은 예언 + 꿀팁 하나. 사람 이름은 **볼드**로.`;
  }
  if (section === '단체에겐테토') {
